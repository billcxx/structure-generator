...
phaseCeramic(:,:,:) = 0
    ...
    rr0 = 8  !radius of ceramics particle
    hCoat = rr0*0.2 !thickness of the coating layer
    particle_num = 800 !total particle number
    ns = 1
    ntry = 1

!   call create_random(sphere0) ！给出一个球的位置和大小
...
!       call create_random(sphere0) ！给出其他球的位置和大小
        sphere0(1) = rd(1+ntry)*(nx-0*rr) !+rr
        sphere0(2) = rd(2+ntry)*(ny-0*rr) !+rr
        sphere0(3) = rd(3+ntry)*(nz-0*rr) !+rr
        sphere0(4) = rr0
        ntry = ntry + 1
        nflag = 0
        do i=1,ns
...
!              nflag = nflag + 1
           ...
           if(ns>10.or.ntry>=29999900) goto 112
        enddo
!       ntry = 0
        ns = ns + 1
...
！判断球的位置是否合适，不合适的丢掉。通过判断两球距离是否大于两球半径和。
112  do i=1,Nx
        do j=1,Ny
            do k=1,Nz
                do m=1,particle_num
...
!Expand the volume by 8 times ！构建超胞，已经跳过（goto 222）
!volume 1
114     do i=1,Nx
                do j=1,Ny
                        do k=1,Nz
...
222 vfCoat = 0.0    ！写结构画图
    ...
    SUBROUTINE create_random(sphere0)   ！生成随机数子程序
    INTEGER :: clock,i,n
